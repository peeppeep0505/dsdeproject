...
def main():
    st.set_page_config(page_title="Authorship", layout="wide")
    st.title("Authorship Connections Across Years")
    # Create tabs
    ...
    with tab_viz:
        ...
        with st.sidebar:
            ...
            if uploaded_files:
                ...
                selected_file_name = st.sidebar.selectbox("Choose a file to visualize", file_names)
                    
                # Get the selected file
                selected_file = next(file for file in uploaded_files if file.name == selected_file_name)
                df = pd.read_csv(selected_file, engine="python", sep=",", quotechar='"', on_bad_lines="skip")
                if len(df.columns) < 2:
                ...
                st.sidebar.warning("Upload at least one CSV file.")
                st.write("Awaiting file uploads...")
